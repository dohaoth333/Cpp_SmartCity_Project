...
Renderer::Renderer(int width, int height)
    ...
    // Initialiser les textures à vide
    vehicleCarTexture = {0};
    ...
    // Initialiser les textures de carte
    tilemapTexture = {0};
    ...
    tileSize = 32; // Taille par défaut des tuiles
...
void Renderer::initialize() {
    ...
    // Charger une police personnalisée si disponible
    ...
    if (!fontLoaded) {
        // Utiliser la police par défaut mais avec meilleur rendu
        gameFont = GetFontDefault();
    ...
    // Améliorer l'espacement des lignes pour un meilleur rendu
    SetTextLineSpacing(2);
    
    // Charger les textures après l'initialisation de la fenêtre
    loadTextures();
    loadMapTiles();
...
void Renderer::cleanup() {
    ...
    // Décharger la musique si elle a été chargée
    if (musicLoaded) {
    ...
    // Décharger la police si elle a été chargée
...
void Renderer::beginFrame() {
    if (!IsWindowReady()) {
        return; // Ne pas essayer de dessiner si la fenêtre n'est pas prête
    }
    BeginDrawing();
    // Changer le fond en vert clair si les tiles sont chargées, sinon noir
    if (mapTilesLoaded && grassTexture.id != 0) {
        ClearBackground({34, 139, 34, 255}); // Vert (herbe)
    } else {
        ClearBackground(backgroundColor); // Noir par défaut
    }
    BeginMode2D(camera);
}
...
void Renderer::endFrame() {
    ...
    EndDrawing();
        ...
        // Ignorer les erreurs de fin de frame
    }
}

void Renderer::renderSimulation(const Simulation& simulation) {
    // ORDRE DE RENDU CORRIGÉ : Routes → Maisons → Véhicules → Événements
    // Pour que les véhicules passent DEVANT les maisons (pas au-dessus)
    
    try {
        const Graph* graph = simulation.getGraph();
        if (graph) {
            renderGraph(*graph);  // Routes + Maisons
        }
    } catch (...) {
        // Ignorer les erreurs de rendu du graphe
    }
    
    try {
        // Rendre les véhicules APRÈS les maisons pour qu'ils passent devant
        renderVehicles(simulation.getVehicles());
    } catch (...) {
        // Ignorer les erreurs de rendu des véhicules
    }
    
    try {
        const Graph* graph = simulation.getGraph();
        if (graph) {
            // Rendre les événements en dernier (au-dessus de tout)
            renderEvents(simulation.getEvents(), *graph);
        }
    } catch (...) {
        // Ignorer les erreurs de rendu des événements
    }
    
    try {
    renderUI(simulation);
    } catch (...) {
        // Ignorer les erreurs de rendu de l'UI
    }
...
void Renderer::renderGraph(const Graph& graph) {
    // Fond réaliste : terrain urbain avec animation de jour/nuit AMÉLIORÉE
    float currentTime = GetTime();
    float dayCycle = sinf(currentTime * 0.1f) * 0.3f + 0.7f; // Cycle jour/nuit subtil
    // Fond plus contrasté pour mieux voir les routes
    Color bgColor = {
        (unsigned char)(50 * dayCycle + 20),   // Plus sombre
        (unsigned char)(100 * dayCycle + 30),  // Plus vert
        (unsigned char)(50 * dayCycle + 20),   // Plus sombre
        255
    };
    ClearBackground(bgColor);
    
    // Rendre les routes avec un style moderne et clair
    for (const auto& route : graph.getRoutes()) {
        ...
        // Vecteur perpendiculaire normalisé pour les bordures
...
void Renderer::loadTextures() {
    ...
    // Charger les nouvelles textures des événements
    ...
    // Vérifier si au moins une texture est chargée
...
void Renderer::loadMusic() {
    // Chemins possibles pour la musique
...
void Renderer::loadMapTiles() {
    ...
    // Essayer de charger le tilemap complet (Roguelike Modern City)
    ...
    for (const auto& file : tilemapFiles) {
        if (FileExists(file.c_str())) {
            ...
            if (tilemapTexture.id != 0) {
                ...
                tileSize = 16; // Selon Tilesheet.txt
                break;
            }
        }
    }
    
    // Charger des tiles individuelles pour les routes
    // D'abord essayer le nouveau dossier assets/routes avec noms français, puis autres
...
void Renderer::renderMapBackground(const Graph& graph) {
    ...
    // Ajouter une marge plus grande pour un meilleur design
    minX -= 300;
    maxX += 300;
    minY -= 300;
    maxY += 300;
    
    // Remplir avec des tuiles d'herbe (alignées avec la grille du graphe)
    if (grassTexture.id != 0) {
        float tileWorldSize = 100.0f; // Aligné avec l'espacement du graphe (100.0f dans createTestGraph)
        // Commencer à un multiple de tileWorldSize pour un alignement parfait
        ...
        // Fallback : remplir avec couleur verte (herbe)
